...
def create_user(command: commands.CreateUser, uow: UnitOfWork) -> None:
    with uow:
        new_user = User(
            command.id,
            command.username,
...
def add_app_auth_to_user(command: commands.AddAuthorizationToUser, uow: UnitOfWork) -> None:
    with uow:
        user = uow.users.get(command.user_id)
        user.add_app_auth(command.app)
